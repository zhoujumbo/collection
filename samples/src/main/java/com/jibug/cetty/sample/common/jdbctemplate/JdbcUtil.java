...
public class JdbcUtil {
     ...
     *             Statement
     */
    public static PreparedStatement createPreparedStatement(final Connection connection,String query, final int batchSize, final int maxRows) throws SQLException {
        // TYPE_FORWARD_ONLY 而默认的TYPE_FORWARD_ONLY参数只允许结果集的游标向下移动。 在从ResultSet（结果集）中读取记录的时，对于访问过的记录就自动释放了内存。
        // ResultSet.CONCUR_READ_ONLY 就是类似只读 属性，不可仪更改的啊！不能用结果集更新数据。
    ...
    public static ResultSet executePreparedStatement(PreparedStatement statement) throws SQLException {
        boolean resultSetAvailable = statement.execute(); // 如果是查询的话返回true，如果是更新或插入的话就返回false了；
        while (!resultSetAvailable && statement.getUpdateCount() != -1) {
            resultSetAvailable = statement.getMoreResults(); // Statement提供了一个getMoreResults()的方法，该方法能将当前Statement "指针" 移动到下一个结果集.
        }
        if (resultSetAvailable) {
